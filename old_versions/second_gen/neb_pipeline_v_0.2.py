...
def optimise_reactants(charge=0, mult=1, trial=0, upper_limit=5, solvent="",xtb=True):
    ...
    trial += 1
    print(f" This is the  {trial} trial")
...
def NEB_TS(charge=0, mult=1,  trial=0, Nimages=16, upper_limit=5, xtb=True,fast=True,solvent="",switch=False):
    ...
    if status == 'COMPLETED' and grep_output('HURRAY', f'{neb_input_name.rsplit(".",1)[0]}.out'):
        ...
        if imag_freq:
            ...
            print("No negative frequency found or are above -50 cm**-1")
            if not xtb and fast:

                print("R2SCAN-3c fast-neb-ts version did not find a TS")
                print("Try with regular NEB-TS and r2scan-3c, take fast HEI as guess")

                subprocess.run("mv neb-fast-TS_NEB-HEI_converged.xyz guess.xyz",shell=True)

                subprocess.run("rm pmix* *densities* freq.inp slurm* neb*im* *neb*.inp",shell=True)
                
                return NEB_TS(charge=charge,mult=mult,Nimages=10,trial=1,upper_limit=MAX_TRIALS,xtb=False,fast=False,solvent=solvent,switch=False)

            if not xtb and not fast:
                print(f"NEB-TS R2scan-3c did not find a TS")
                print(f"check if NEB-guess has a significant mode")
                imag_freq=freq_job(struc_name=f'{neb_input_name.rsplit(".",1)[0]}_NEB-CI_converged.xyz', charge=charge, mult=mult, trial=0, upper_limit=upper_limit, solvent=solvent, xtb=xtb, ts=True)
                if imag_freq:
                    print("guess has a mode below 50 cm-1. Continue with TS_opt")
                    subprocess.run(f"cp {neb_input_name.rsplit('.',1)[0]}_NEB-TS_converged.xyz neb_completed.xyz", shell=True)
                    os.chdir('..')
                    return True
            ...
            if  xtb and fast:
                print("Try with regular NEB-TS and xtb. Take fast HEI as guess")

                subprocess.run("mv neb-fast-TS_NEB-HEI_converged.xyz guess.xyz",shell=True)


                subprocess.run("rm pmix* *densities* freq.inp slurm* neb*im* *neb*.inp",shell=True)
                return NEB_TS(charge=charge,mult=mult,trial=2,upper_limit=MAX_TRIALS,xtb=True,fast=False,solvent=solvent,switch=False)
            elif xtb and not fast:



                print(f"xtb runs failed after {trial} trials")
                print(f" Retry NEB-TS with r2scan-3c and 10 images, take xtb as guess TS")

                subprocess.run("mv neb-TS_NEB-CI_converged.xyz guess.xyz",shell=True)
                subprocess.run("rm pmix* *densities* freq.inp slurm* neb*im* *neb*.inp",shell=True)



                     
                return NEB_TS(charge=charge,Nimages=10,mult=mult,trial=2,upper_limit=MAX_TRIALS,xtb=False,fast=False,solvent=solvent,switch=True)
        ...
        return NEB_TS(charge=charge, mult=mult, trial=trial,Nimages=images ,  upper_limit=upper_limit, xtb=xtb, fast=fast, solvent=solvent,switch=switch)
...
def pipeline(step,charge=0, mult=1, solvent="", Nimages=16):
    ...
    match step:
        ...
        case "NEB-TS_XTB":
            return NEB_TS(charge, mult, trial=0, Nimages=Nimages, upper_limit=MAX_TRIALS, xtb=True, fast=False, solvent=solvent)
        case "NEB-TS_DFT":
